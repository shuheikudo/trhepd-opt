...
!*******************************************************************
        subroutine bulkio(inegpos,idiag,iprn)
          ...
          real(8) :: ghx,ghy,gky,da1,sap,sae, amass,zmin,zmax,zoffset,zi
          integer :: inegpos,isym,nh,nk,ndom,ml,nelm,nsg,mp,natm,naz,ng,ns,ngr,nv
          integer :: nbt,ibt,nabr,nabi,iabr,iabp,iabe,idiag
          integer :: iprn, idom,iz,ichg, i,j
          complex(8), dimension(:,:,:),allocatable :: st
          ...
          do i=1,ndom
            ...
            print *, (ih(j),ik(j),j=ibt+1,ibt+nb(i))
            ibt=ibt+nb(i)
          end do
          ...
          do idom=1,ndom
            ...
            ! in ns st(:,:,1), below ns st(:,:,2), above ns st(:,:,3)
            call scpot_stf(natm,nv,nsg,1,0,0,1,gh,gk,x,y,0d0,0d0,st(1,1,1))
            call scpot_stf(natm,nv,nsg,1,0,0,1,gh,gk,x,y,-dx,-dy,st(1,1,2))
            call scpot_stf(natm,nv,nsg,1,0,0,1,gh,gk,x,y,dx,dy,st(1,1,3))
            
            ! compute potentials
            ! top (in ns) -> lower (below ns) -> upper (above ns)
            do i=1,ns
              zi = dz*(i-0.5d0)
              call scpot(nv,nv,v(1,i),vi(1,i),natm,nelm,ielm,z,zi,mab,nabr,nabi,asf,b,dchg,1d0,Ghk,ocr,st(1,1,1))
              call scpot(nv,nv,v(1,i),vi(1,i),natm,nelm,ielm,z,zi+cc,mab,nabr,nabi,asf,b,dchg,1d0,Ghk,ocr,st(1,1,2))
              call scpot(nv,nv,v(1,i),vi(1,i),natm,nelm,ielm,z,zi-cc,mab,nabr,nabi,asf,b,dchg,1d0,Ghk,ocr,st(1,1,3))
            end do
